...
public class Grammar {
    ...
    private void readFromFile(String filename) {
        try{
            ...
            // first and last lines for productions will not contain any relevant information, we only need to check
            // starting from the second until the second-last
            reader.readLine();
    ...
    public List<Production> getProductions(){
        if ( productions.size() == 0) {
            for (Set<String> lhs : P.keySet()) {
                for (String key : lhs) {
                    List<List<String>> rule = new ArrayList<>(P.get(lhs));
                    Production production = new Production(key, rule);
    ...
    public List<Production> getProductionListForNonterminal(String nonTerminal) {
    ...
    public Set<Production> getProductionsContainingNonterminal(String nonTerminal){
